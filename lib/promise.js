...
var slice = function (arr, start, end) { return Array.prototype.slice.call(arr, start, end) };
...
Promise.prototype.onFulfill = function (fn) {
  ...
  if (!this.listeners(this.constructor.SUCCESS).length) {
    this.fulfill = fn;
  } else {
    this.fulfill = Promise.prototype.fulfill;
  }
...
Promise.prototype.onReject = function (fn) {
  if (!fn) return this;
  if ('function' != typeof fn) throw new TypeError("fn should be a function");
  if (!this.listeners(this.constructor.FAILURE).length) {
    this.reject = fn;
  } else {
    this.reject = Promise.prototype.reject;
  }
...
Promise.prototype.onResolve = function (fn) {
  if (!fn) return this;
  if ('function' != typeof fn) throw new TypeError("fn should be a function");
  if (!this.listeners(this.constructor.FAILURE).length && !this.listeners(this.constructor.SUCCESS).length) {
    this.fulfill = function () {var args = slice(arguments); args.unshift(null); fn.apply(this, args)};
    this.reject = fn;
  } else {
    this.fulfill = Promise.prototype.fulfill;
    this.reject = Promise.prototype.reject;
  ...
  return function handler () {
    ...
    process.nextTick(
      function in_the_handler() {
        if (retPromise.domain) retPromise.domain.enter();
        var x;
